...
internal sealed class ConsoleSecurityWindow : Window
{
    ...
    public ConsoleSecurityWindow(string username)
    {
        ...
        panel.Children.Add(new TextBlock { Text = "新密码", Margin = new Thickness(0, 12, 0, 4) }); passwordBox.Height = 31; panel.Children.Add(passwordBox);
        ...
        save.Click += delegate { if (Username == "" || Password == "") { MessageBox.Show("请填写用户名和新密码。", Program.AppName); return; } if (Password != confirmBox.Password) { MessageBox.Show("两次密码不一致。", Program.AppName); return; } DialogResult = true; };
