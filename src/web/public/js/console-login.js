(() => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const secureProof = await proof(password, username, challenge);
      await request("/api/console-auth/login", { method: "POST", body: JSON.stringify({ challengeId: challenge.challengeId, username, proof: secureProof, password: secureProof ? undefined : password, remember: document.getElementById("remember").checked }) });
